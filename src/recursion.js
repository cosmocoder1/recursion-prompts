...
var isEven = function(n) {
  ...
  //credit due to members of Stack Overflow for inspiration on this one
  if (n === 0) {
    return true;
  } else if (n === 1) {
    return false;
...
var exponent = function(base, exp) {
    ...
    if (exp < 0) {
      posExp = exp.toString().split('');
      posExp.shift('');
      newExp = parseInt(posExp.join(''))
      accumulator *= exponent(base, newExp - 1);
...
var reverse = function (string) {
  ...
  if (string === "") {
    ...
    return reverse(string.split('').slice(1).join('')) + string.charAt(0);
}
  ...
  // handle base case
   if (string.length < 2) {
    ...
    if (string.charAt(0).toLowerCase() === string.charAt(string.length - 1).toLowerCase()) {
      return palindrome(string.split('').slice(1, string.length - 1).join(''));
    }
...
var divide = function(x, y) {
  ...
  return divide(positive(x) - positive(y), positive(y)) + 1;
    }
  }
...
var createArray = function (string) {
  ...
  var result = [];
    ...
    return result.concat(createArray(string.substr(1)));
